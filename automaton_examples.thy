...
theorem non_reg_lang_is_non_regular:
  assumes "infinite (UNIV :: 's set)"
  shows "non_reg_lang \<notin> regular_languages {0, 1}"
proof(rule ccontr)
  assume "\<not> non_reg_lang \<notin> regular_languages {0, 1}"
  hence "non_reg_lang \<in> regular_languages {0, 1}" by blast
  hence "\<exists> n > 0 . \<forall> word \<in> non_reg_lang . length word \<ge> n \<longrightarrow> (\<exists> u v w . word = u @ v @ w \<and> length (u @ v) \<le> n \<and> length v \<ge> 1 \<and> (\<forall> i . (u @ (pump v i) @ w) \<in> non_reg_lang))" using assms pumping_lemma by fast
