...
text \<open>Last updated: 10.11.2025\<close>
...
proposition sigma_star_auto_language_right:
  assumes "finite \<Sigma>"
  shows "word \<in> sigma_star \<Sigma> \<Longrightarrow> word \<in> language_auto (\<A>_sigma_star \<Sigma>)"
proof(induction word rule: rev_induct)
  case Nil
...
next
  ...
  hence prun: "pseudo_run_well_defined run (\<A>_sigma_star \<Sigma>) (initial_state (\<A>_sigma_star \<Sigma>)) word \<and> last run = 0 \<and> (0, a, 0) \<in> transitions (\<A>_sigma_star \<Sigma>)" unfolding \<A>_sigma_star_def by simp
  have "auto_well_defined (\<A>_sigma_star \<Sigma>)" using assms \<A>_sigma_star_well_def by blast
  hence "pseudo_run_well_defined (run @ [0]) (\<A>_sigma_star \<Sigma>) (initial_state (\<A>_sigma_star \<Sigma>)) (word @ [a])" using prun prun_well_defined_extension_snoc by fastforce
